...
for subdir in tqdm(os.listdir(input_dir),position=0):
    ...
    for file in tqdm(text_files,position=1):
        if file.endswith(caption_ext):
            ...
            # # ori_result = hpsv2.score(ori_image, prompt, hps_version="v2.1")[0]
            # ori_result = aesthetic_predict.predict(ae_model,image_encoder,preprocess,ori_image,device)
            # result['ori_result']=float(ori_result)
            # # pag_result = hpsv2.score(pag_image, prompt, hps_version="v2.1")[0]
            # pag_result = aesthetic_predict.predict(ae_model,image_encoder,preprocess,pag_image,device)
            # result['pag_result']=float(pag_result)

            # if ori_result > pag_result:
            #     result['high'] = 'ori'
            #     result['low'] = 'pag'
            #     ori_image.save(output_high_file_path)
            #     pag_image.save(output_low_file_path)
            # else:
            #     result['high'] = 'pag'
            #     result['low'] = 'ori'
            #     ori_image.save(output_low_file_path)
            #     pag_image.save(output_high_file_path)
            # result['high_file'] = output_high_file_path
            # result['low_file'] = output_low_file_path
            # print(result)
            # results.append(result)
            
            # if count % save_json_every_progress == 0 or count == len(text_files):
