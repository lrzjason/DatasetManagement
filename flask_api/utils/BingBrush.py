...
class BingBrush:
    ...
    def process(self, prompt):
        ...
        #     if 'http' in url:
        #         out_name = f"{prompt}_{idx}"
        #         self.write_image(url,out_folder,out_name)
        # print(f"==> Images are saved to {out_folder}")

    def write_image(self, url, out_folder, out_name=""):
        response = requests.get(url)
        if response.status_code == 200:
            file_name = url.split("/")[-1]
            if out_name != "":
                file_name = out_name
            save_path = os.path.join(out_folder, file_name) + ".webp"
...
def main():
    cookie = 'F:/ImageSet/Dalle3/cookie.txt'
    output_dir = 'F:/ImageSet/Dalle3/'
    brush = BingBrush(cookie=cookie)
    img_urls = brush.process(prompt='a girl')